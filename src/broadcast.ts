...
/**
 * Fires a throwaway eth_chainId request to each RPC endpoint and discards
 * the result. Purpose: pre-establish the TCP/TLS connection (and let
 * connection-pooling keep it warm) so the *real* broadcast later doesn't
 * pay handshake latency in the critical path. Errors are ignored — this
 * is best-effort only.
 */
export function warmUp(rpcUrls: string[]): void {
